...
table_name = 'navio_frigorifico'
navio_frigorifico_df = pd.read_sql_table(table_name, engine)

# Exibir as primeiras linhas do DataFrame para verificar o carregamento
print(navio_frigorifico_df.head())
...
navio_frigorifico_clean_df = remove_outliers(navio_frigorifico_df, 'Valor (USD)')

# Medidas de dispersão e posição
measures_of_dispersion = navio_frigorifico_clean_df['Valor (USD)'].describe()
print(measures_of_dispersion)

# Adicionar mais medidas de dispersão
measures_of_dispersion['variance'] = navio_frigorifico_clean_df['Valor (USD)'].var()
...
avg_cost_per_ship_clean = navio_frigorifico_clean_df.groupby('Nome do Navio')['Valor (USD)'].mean()

# Custos médios por porto
avg_cost_per_port_clean = navio_frigorifico_clean_df.groupby('Porto')['Valor (USD)'].mean()

# Custos médios por tipo de custo
avg_cost_per_cost_type_clean = navio_frigorifico_clean_df.groupby('Tipo de Custo')['Valor (USD)'].mean()

# Gráfico de custos médios por navio
plt.figure(figsize=(10, 6))
sns.barplot(x=avg_cost_per_ship_clean.index, y=avg_cost_per_ship_clean.values)
plt.xticks(rotation=90)
plt.title('Custos Médios por Navio (Navio Frigorífico, sem outliers)')
...
plt.xticks(rotation=90)
plt.title('Custos Médios por Porto (Navio Frigorífico, sem outliers)')
plt.ylabel('Valor Médio (USD)')
plt.xlabel('Porto')
plt.show()
...
plt.xticks(rotation=90)
plt.title('Custos Médios por Tipo de Custo (Navio Frigorífico, sem outliers)')
plt.ylabel('Valor Médio (USD)')
...
dispersion_df.to_csv('medidas_dispersao_frigorifico.csv', index=False)

# Salvar o DataFrame em uma tabela SQL
dispersion_df.to_sql('medidas_dispersao_frigorifico', con=engine, if_exists='replace', index=False)