...
table_name = 'navio_graneleiro'
navio_graneleiro_df = pd.read_sql_table(table_name, engine)

# Exibindo as primeiras linhas do DataFrame para verificar o carregamento
print(navio_graneleiro_df.head())
...
navio_graneleiro_clean_df = remove_outliers(navio_graneleiro_df, 'Valor (USD)')

# Medidas de dispersão e posição
measures_of_dispersion = navio_graneleiro_clean_df['Valor (USD)'].describe()

# Adicionar mais medidas de dispersão
measures_of_dispersion['variance'] = navio_graneleiro_clean_df['Valor (USD)'].var()
...
avg_cost_per_ship_clean = navio_graneleiro_clean_df.groupby('Nome do Navio')['Valor (USD)'].mean()

# Custos médios por porto
avg_cost_per_port_clean = navio_graneleiro_clean_df.groupby('Porto')['Valor (USD)'].mean()

# Custos médios por tipo de custo
avg_cost_per_cost_type_clean = navio_graneleiro_clean_df.groupby('Tipo de Custo')['Valor (USD)'].mean()

# Gráfico de custos médios por navio
plt.figure(figsize=(10, 6))
sns.barplot(x=avg_cost_per_ship_clean.index, y=avg_cost_per_ship_clean.values)
plt.xticks(rotation=90)
plt.title('Custos Médios por Navio (Navio Graneleiro, sem outliers)')
...
plt.xticks(rotation=90)
plt.title('Custos Médios por Porto (Navio Graneleiro, sem outliers)')
plt.ylabel('Valor Médio (USD)')
plt.xlabel('Porto')
plt.show()
...
plt.xticks(rotation=90)
plt.title('Custos Médios por Tipo de Custo (Navio Graneleiro, sem outliers)')
plt.ylabel('Valor Médio (USD)')
